...
extension ListTypeExtensions on List<Type> {
  Map<String, double> get damageFrom {
    ...
    forEach((type) {
      for (final it in type.damageRelations.noDamageFrom) {
      ...
      for (final it in type.damageRelations.halfDamageFrom) {
        if (damage.containsKey(it.name)) {
      ...
      for (final it in type.damageRelations.doubleDamageFrom) {
        if (damage.containsKey(it.name)) {
